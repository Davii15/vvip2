...
export default function HealthServicesMediaShowcase() {
  useCookieTracking("health-services-media")
 ...
 const [showFilters, setShowFilters] = useState(true)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-900 via-blue-800 to-teal-900">
      {/* Header */}
      <div className="sticky top-0 z-50 bg-gradient-to-r from-cyan-900 to-blue-900 border-b border-cyan-700 shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="text-center md:text-left">
              <h1 className="text-2xl md:text-3xl font-bold text-cyan-100">Health Services Media</h1>
              <p className="text-cyan-200 text-sm md:text-base">
                Discover how healthcare providers deliver exceptional medical services
              </p>
             <Button 
                variant="ghost" 
                size="sm" 
              ...
              <Button
                ...
                className={cn(
                  ...
                  activeCategory === null
                    ...
                    : "border-cyan-500 text-cyan-200 hover:bg-cyan-900/50",
                )}
              >
                All
              </Button>
      ...
      <div className="container mx-auto px-4 py-8" ref={containerRef}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVideos.map((video, index) => (
            <div
              key={video.id}
              className="relative bg-gradient-to-b from-cyan-900/90 to-blue-900/90 rounded-xl overflow-hidden shadow-xl border border-cyan-600/30 backdrop-blur-sm"
              ref={(el) => {
            ...
            >
              ...
              <div className="relative aspect-[9/16] bg-black">
                ...
                <div className="absolute top-3 right-3">
                  <Badge
                    variant="outline"
                    className="border-cyan-500 text-cyan-100 bg-black/50 flex items-center gap-1"
                  >
              ...
              <div className="p-4">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1">
                      <h3 className="font-semibold text-cyan-100 truncate">{video.vendor.name}</h3>
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-cyan-200 hover:text-cyan-100 hover:bg-cyan-800/50"
                ...
                <h2 className="font-bold text-cyan-100 mb-2 line-clamp-2">{video.title}</h2>
                ...
                <AnimatePresence>
                  {showInfo[video.id] ? (
                    ...
                    >
                      <p className="text-sm text-cyan-200 mb-3">{video.description}</p>

                      {/* Tags */}
                      <div className="flex flex-wrap gap-1 mb-3">
                        {video.tags.map((tag) => (
                          <Badge key={tag} variant="outline" className="text-xs border-cyan-700 text-cyan-300">
                      ...
                      {/* Vendor details */}
                      <div className="bg-cyan-900/50 rounded-lg p-3 mb-3">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-cyan-200">Rating:</span>
                          <div className="flex items-center">
                            ...
                            <span className="ml-1 text-xs text-cyan-200">
                              {video.vendor.rating.toFixed(1)} ({video.vendor.reviewCount})
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-cyan-200">Followers:</span>
                    ...
                    <p className="text-sm text-cyan-200 mb-3 line-clamp-2">{video.description}</p>
                  )}
                </AnimatePresence>

                {/* Video stats and actions */}
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-cyan-800">
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      className={cn(
                        ...
                        isLiked[video.id] ? "text-red-500 hover:text-red-600" : "text-cyan-200 hover:text-cyan-100",
                      )}
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-8 px-2 text-cyan-200 hover:text-cyan-100 flex items-center gap-1"
                    >
                  ...
                  <div className="flex items-center">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-cyan-200 hover:text-cyan-100">
        ...
        {isLoading && (
          <div className="flex justify-center items-center py-8">
            <div className="bg-cyan-900/80 p-4 rounded-full backdrop-blur-sm flex items-center gap-2">
              <Loader2 className="h-5 w-5 animate-spin text-cyan-300" />
              <span className="text-cyan-200 font-medium">Loading more videos...</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-4 right-4 z-40 flex flex-col gap-2">
        ...
        {showScrollToTop && (
          <motion.button
            ...
            className="bg-cyan-600 hover:bg-cyan-700 text-white p-3 rounded-full shadow-lg"
            onClick={scrollToTop}
          >
        ...
        {/* Mute/unmute button */}
        <button className="bg-cyan-600 hover:bg-cyan-700 text-white p-3 rounded-full shadow-lg" onClick={toggleMute}>
