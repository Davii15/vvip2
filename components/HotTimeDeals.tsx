...
export default function HotTimeDeals({
  deals,
  colorScheme,
  title = "Hot Time-Limited Deals",
  subtitle = "Grab these exclusive offers before they expire!",
}: HotTimeDealsProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [visibleDeals, setVisibleDeals] = useState<HotDeal[]>([])
      ...
      <div className="relative overflow-hidden rounded-lg bg-white p-4 sm:p-6">
        ...
        <div className="block md:hidden">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="w-full"
            >
              <HotDealCard deal={visibleDeals[currentIndex]} colors={colors} />
            </motion.div>
          </AnimatePresence>
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {visibleDeals.slice(0, 4).map((deal) => (
            <HotDealCard key={deal.id} deal={deal} colors={colors} />
          ))}
        </div>
...
function HotDealCard({ deal, colors }: { deal: HotDeal; colors: any }) {
    ...
    >
      ...
      <div className="p-4 flex-grow flex flex-col">
        ...
        <div className="flex items-end justify-between mt-auto">
          <div>
            <div className={`text-lg font-bold ${colors.highlight}`}>{formatPrice(deal.currentPrice)}</div>
